...
## Phase 25 — bidirectional BWT exact interval extension — ACTIVE FRONTIER

- [ ] Maintain coordinated exact suffix-array intervals for a pattern in the forward text and its reversal in the reversed text, support arbitrary-byte extension on either the left or the right from one common empty state, preserve paired interval cardinality invariants, and verify every extension sequence/count against independent direct scan. A first-principles bounded-byte-alphabet baseline may perform explicit alphabet work per extension; it must reuse the sealed BWT occurrence machinery and make no r-index, approximate-matching, compressed-construction, or asymptotically optimal bidirectional-index claim.
