...
## Phase 27 — bounded edit-distance approximate BWT search — ACTIVE FRONTIER

- [ ] Exact substring start-position search under a bounded byte-oriented Levenshtein budget, extending the sealed BWT state machine with substitution, insertion, and deletion transitions. Production must retain deterministic replayable state/branch diagnostics, deduplicate dominated equivalent search states without direct scanning, reconstruct exact start-position witnesses through the sealed BWT locate machinery, and make the exponential-in-budget baseline cost explicit. Verification must compare against an independent direct dynamic-programming substring oracle. No seed-and-extend, affine-gap, probabilistic, or polynomial-time approximate-index claim is implied.
