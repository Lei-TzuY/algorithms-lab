...
## Phase 51 — scratch-aware backend register reservation — SEALED

- [x] Given a finite physical-register file, deterministically search reserved spill-scratch capacity while rerunning sealed Phase-49 coalescing and Phase-50 spill materialization under the reduced allocatable budget. Select the smallest reservation whose measured abstract scratch demand fits the reserved suffix, map every used abstract scratch index to a non-overlapping real physical register, expose reservation/feasibility provenance, and verify that assigned registers and scratch registers are disjoint on every backend operation. Keep byte-level frame layout/alignment, concrete ISA instruction selection/addressing, calling conventions, minimum-spill/global-register optimality, MemorySSA, and post-rewrite allocation optimization outside this slice.

## Phase 52 — target-neutral byte-addressed spill-frame layout — SEALED

- [x] Convert the sealed Phase-51 scratch-aware selection into a deterministic byte-addressed spill-frame view: assign every dense abstract stack slot one aligned, non-overlapping byte offset using explicit slot-size/alignment and whole-frame-alignment parameters; preserve all physical-register ids and operation/block provenance; map every class/location/reload/store stack reference through the same slot-to-offset table; expose total frame size; and reject invalid alignment or checked-arithmetic overflow. Keep stack-growth direction, signed base-relative addressing, concrete ISA opcodes/addressing modes, frame-pointer choice, calling conventions, prologue/epilogue, slot coloring/reuse, and ABI-minimal-frame claims outside this slice.

## Phase 53 — signed base-relative frame addressing — ACTIVE FRONTIER

- [ ] Lower sealed Phase-52 `frame_byte_offset` storage into a target-neutral signed base-relative displacement view with an explicit logical frame-base byte anchor and caller-supplied representable displacement interval. Preserve physical-register ids plus every class/location/block/operation provenance field, require every memory displacement to be derived exactly from the sealed slot offset and base anchor with checked signed arithmetic, and reject malformed frames, out-of-frame anchors, or displacements outside the configured range. Keep concrete ISA opcode selection, addressing-mode encoding, stack/base-register assignment, calling conventions, prologue/epilogue, red zones, variable-size objects, and ABI-specific frame policy outside this slice.
