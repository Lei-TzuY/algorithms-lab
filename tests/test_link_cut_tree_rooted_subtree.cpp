#include "algorithms/graphs/link_cut_tree.hpp"
#include "test_framework.hpp"

#include <algorithm>
#include <cstddef>
#include <cstdint>
#include <random>
#include <stdexcept>
#include <utility>
#include <vector>

namespace {

using algorithms::graphs::LinkCutForest;
using algorithms::graphs::Vertex;
using Adjacency = std::vector<std::vector<unsigned char>>;

std::vector<Vertex> bfs_path(const Adjacency& adjacency, Vertex start,
                             Vertex goal) {
  const Vertex none = static_cast<Vertex>(adjacency.size());
  std::vector<Vertex> parent(adjacency.size(), none);
  std::vector<Vertex> queue;
  parent[start] = start;
  queue.push_back(start);

  for (std::size_t head = 0;
       head < queue.size() && parent[goal] == none; ++head) {
    const Vertex vertex = queue[head];
    for (Vertex next = 0; next < adjacency.size(); ++next) {
      if (adjacency[vertex][next] == 0U || parent[next] != none) {
        continue;
      }
      parent[next] = vertex;
      queue.push_back(next);
    }
  }

  if (parent[goal] == none) {
    return {};
  }
  std::vector<Vertex> path;
  for (Vertex current = goal;; current = parent[current]) {
    path.push_back(current);
    if (current == start) {
      break;
    }
  }
  std::reverse(path.begin(), path.end());
  return path;
}

std::size_t rooted_subtree_size(const Adjacency& adjacency, Vertex root,
                                Vertex vertex) {
  const Vertex none = static_cast<Vertex>(adjacency.size());
  std::vector<Vertex> parent(adjacency.size(), none);
  std::vector<Vertex> queue;
  parent[root] = root;
  queue.push_back(root);

  for (std::size_t head = 0; head < queue.size(); ++head) {
    const Vertex current = queue[head];
    for (Vertex next = 0; next < adjacency.size(); ++next) {
      if (adjacency[current][next] == 0U || parent[next] != none) {
        continue;
      }
      parent[next] = current;
      queue.push_back(next);
    }
  }
  if (parent[vertex] == none) {
    return 0;
  }

  std::size_t count = 0;
  std::vector<std::pair<Vertex, Vertex>> stack{{vertex, parent[vertex]}};
  while (!stack.empty()) {
    const auto [current, previous] = stack.back();
    stack.pop_back();
    ++count;
    for (Vertex next = 0; next < adjacency.size(); ++next) {
      if (adjacency[current][next] != 0U && next != previous) {
        stack.emplace_back(next, current);
      }
    }
  }
  return count;
}

std::vector<std::pair<Vertex, Vertex>> existing_edges(
    const Adjacency& adjacency) {
  std::vector<std::pair<Vertex, Vertex>> edges;
  for (Vertex first = 0; first < adjacency.size(); ++first) {
    for (Vertex second = first + 1; second < adjacency.size(); ++second) {
      if (adjacency[first][second] != 0U) {
        edges.emplace_back(first, second);
      }
    }
  }
  return edges;
}

std::size_t pick(std::mt19937_64& engine, std::size_t bound) {
  return static_cast<std::size_t>(
      engine() % static_cast<std::uint64_t>(bound));
}

std::int64_t small_value(std::mt19937_64& engine) {
  return static_cast<std::int64_t>(engine() % 41U) - 20;
}

std::int64_t small_delta(std::mt19937_64& engine) {
  return static_cast<std::int64_t>(engine() % 7U) - 3;
}

std::int64_t path_value_sum(const std::vector<Vertex>& path,
                            const std::vector<std::int64_t>& values) {
  std::int64_t sum = 0;
  for (const Vertex vertex : path) {
    sum += values[vertex];
  }
  return sum;
}

}  // namespace

TEST_CASE(link_cut_rooted_subtree_counts_chain_star_and_reroot) {
  LinkCutForest chain(5);
  for (Vertex vertex = 0; vertex + 1 < 5; ++vertex) {
    chain.link(vertex, vertex + 1);
  }
  for (Vertex vertex = 0; vertex < 5; ++vertex) {
    REQUIRE_EQ(chain.rooted_subtree_vertex_count(0, vertex), 5 - vertex);
    REQUIRE_EQ(chain.rooted_subtree_vertex_count(4, vertex), vertex + 1);
  }
  REQUIRE(chain.valid_auxiliary_invariants());

  LinkCutForest star(6);
  for (Vertex vertex = 1; vertex < 6; ++vertex) {
    star.link(0, vertex);
  }
  REQUIRE_EQ(star.rooted_subtree_vertex_count(0, 0), 6U);
  for (Vertex vertex = 1; vertex < 6; ++vertex) {
    REQUIRE_EQ(star.rooted_subtree_vertex_count(0, vertex), 1U);
  }
  REQUIRE_EQ(star.rooted_subtree_vertex_count(1, 1), 6U);
  REQUIRE_EQ(star.rooted_subtree_vertex_count(1, 0), 5U);
  REQUIRE_EQ(star.rooted_subtree_vertex_count(1, 2), 1U);
  REQUIRE(star.valid_auxiliary_invariants());
}

TEST_CASE(link_cut_rooted_subtree_rejects_disconnected_and_survives_relink) {
  LinkCutForest forest(6);
  forest.link(0, 1);
  forest.link(1, 2);
  forest.link(1, 3);
  forest.link(3, 4);

  REQUIRE_EQ(forest.rooted_subtree_vertex_count(0, 1), 4U);
  REQUIRE_THROWS_AS(forest.rooted_subtree_vertex_count(0, 5),
                    std::invalid_argument);
  REQUIRE_THROWS_AS(forest.rooted_subtree_vertex_count(6, 0),
                    std::out_of_range);

  forest.cut(1, 3);
  REQUIRE_THROWS_AS(forest.rooted_subtree_vertex_count(0, 4),
                    std::invalid_argument);
  forest.link(2, 4);
  REQUIRE_EQ(forest.rooted_subtree_vertex_count(0, 2), 3U);
  REQUIRE_EQ(forest.rooted_subtree_vertex_count(4, 1), 2U);
  REQUIRE(forest.valid_auxiliary_invariants());
}

TEST_CASE(link_cut_rooted_subtree_is_orthogonal_to_lazy_numeric_state) {
  LinkCutForest forest(6);
  forest.link(0, 1);
  forest.link(1, 2);
  forest.link(1, 3);
  forest.link(3, 4);
  forest.link(3, 5);

  forest.assign_path_value(2, 4, 7);
  forest.add_path_value(0, 5, 3);

  REQUIRE_EQ(forest.rooted_subtree_vertex_count(0, 1), 5U);
  REQUIRE_EQ(forest.rooted_subtree_vertex_count(2, 3), 3U);
  REQUIRE_EQ(forest.path_sum(2, 4), 34);
  REQUIRE(forest.valid_auxiliary_invariants());
}

TEST_CASE(link_cut_rooted_subtree_randomized_against_bfs_oracle) {
  constexpr std::size_t kVertexCount = 12;
  constexpr std::size_t kSteps = 3000;
  LinkCutForest forest(kVertexCount);
  Adjacency adjacency(kVertexCount,
                      std::vector<unsigned char>(kVertexCount, 0U));
  std::vector<std::int64_t> values(kVertexCount, 0);
  std::mt19937_64 engine(0x39A11CEULL);

  for (std::size_t step = 0; step < kSteps; ++step) {
    const Vertex first = pick(engine, kVertexCount);
    const Vertex second = pick(engine, kVertexCount);
    const std::size_t operation = pick(engine, 8);

    if (operation == 0) {
      if (first != second && bfs_path(adjacency, first, second).empty()) {
        forest.link(first, second);
        adjacency[first][second] = 1U;
        adjacency[second][first] = 1U;
      }
    } else if (operation == 1) {
      const auto edges = existing_edges(adjacency);
      if (!edges.empty()) {
        const auto [edge_first, edge_second] =
            edges[pick(engine, edges.size())];
        forest.cut(edge_first, edge_second);
        adjacency[edge_first][edge_second] = 0U;
        adjacency[edge_second][edge_first] = 0U;
      }
    } else if (operation == 2) {
      const std::int64_t value = small_value(engine);
      forest.assign_value(first, value);
      values[first] = value;
    } else if (operation == 3) {
      const auto path = bfs_path(adjacency, first, second);
      if (!path.empty()) {
        const std::int64_t value = small_value(engine);
        forest.assign_path_value(first, second, value);
        for (const Vertex vertex : path) {
          values[vertex] = value;
        }
      }
    } else if (operation == 4) {
      const auto path = bfs_path(adjacency, first, second);
      if (!path.empty()) {
        const std::int64_t delta = small_delta(engine);
        forest.add_path_value(first, second, delta);
        for (const Vertex vertex : path) {
          values[vertex] += delta;
        }
      }
    } else if (operation == 5) {
      const std::size_t expected =
          rooted_subtree_size(adjacency, first, second);
      if (expected == 0) {
        REQUIRE_THROWS_AS(
            forest.rooted_subtree_vertex_count(first, second),
            std::invalid_argument);
      } else {
        REQUIRE_EQ(forest.rooted_subtree_vertex_count(first, second), expected);
      }
    } else if (operation == 6) {
      const auto path = bfs_path(adjacency, first, second);
      if (path.empty()) {
        REQUIRE_THROWS_AS(forest.path_sum(first, second),
                          std::invalid_argument);
      } else {
        REQUIRE_EQ(forest.path_sum(first, second),
                   path_value_sum(path, values));
      }
    } else {
      const auto path = bfs_path(adjacency, first, second);
      if (!path.empty()) {
        const std::size_t rank = pick(engine, path.size());
        REQUIRE_EQ(forest.kth_vertex_on_path(first, second, rank), path[rank]);
      }
    }

    if ((step % 37U) == 0U) {
      REQUIRE(forest.valid_auxiliary_invariants());
    }
  }

  for (Vertex root = 0; root < kVertexCount; ++root) {
    for (Vertex vertex = 0; vertex < kVertexCount; ++vertex) {
      const std::size_t expected =
          rooted_subtree_size(adjacency, root, vertex);
      if (expected == 0) {
        REQUIRE_THROWS_AS(forest.rooted_subtree_vertex_count(root, vertex),
                          std::invalid_argument);
      } else {
        REQUIRE_EQ(forest.rooted_subtree_vertex_count(root, vertex), expected);
      }
    }
  }
  REQUIRE(forest.valid_auxiliary_invariants());
}
