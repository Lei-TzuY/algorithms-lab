...
// the visited block only from program.start and sealed semantic control.
...
// sealed semantic-control block executor and follows only its explicit concrete
...
// between visits using the sealed exact-handoff invariant.
//
// visit_requests.size() must equal visit_budget and visit_budget must be nonzero.
// The executor may stop before consuming all requests when a body suspends,
// control remains opaque, or an explicit return_void terminator completes. If a
// successor is selected on the final allowed visit, stop_reason is
// visit_budget_exhausted and next_execution_block records the exact concrete
// block that would execute next.
//
// On returned, production reconstructs the canonical pre-exit register state and
// executes the sealed fixed-frame exit sequence exactly once. The replay witness
// is present only for returned execution. Body suspension, opaque control, and
// finite budget exhaustion remain non-terminal and never execute fixed-frame
// exit. This API adds no return value, general memory, call, exception, ABI, or
// target-ISA semantics.
