...
The post-Phase-69 scope-recovery policy remains authoritative; the historical
compiler/backend ROADMAP tail is intentionally not used as a prospective
frontier.

A fresh live audit on 2026-09-16 found that deterministic order-statistic
selection was still absent from `main`, but it also found an abandoned
`scope-recovery-bfprt-selection` branch from 2026-09-12. That branch contained a
coherent BFPRT implementation, native tests, and this proof note, but had no PR
CI and had fallen 108 commits behind live `main`. Rather than creating a
parallel implementation, this checkpoint replays that existing candidate onto
exact green `main@d754cc2c5ca862180227235d621c51fd851faa56`, updates only the
current test registration surface, and reruns focused compiler/sanitizer
evidence before remote full-suite validation.

This slice changes proof model again. Recent recovery work spans subset-sum
time/space tradeoffs, graphical realization, Robinson-Schensted correspondence,
optimal alphabetic coding, lattice reduction, streaming sketches, and
comparison-efficient sorting. Deterministic selection instead adds a classical
...
median-of-medians pivot, at least three group elements are `<= pivot`;
symmetrically, for every qualifying high-side group at least three are
`>= pivot`. Discarding a constant number of exceptional groups for the pivot
group and a possible final partial group still removes at least
`3n/10 - O(1)` elements from either strict side. Therefore the worst-case
recurrence has the classical form
...
Focused replay on the current test framework passed:
