#pragma once

#include "algorithms/strings/bwt_index.hpp"

#include <algorithm>
#include <cstddef>
#include <cstdint>
#include <limits>
#include <stdexcept>
#include <string_view>
#include <vector>

namespace algorithms::strings::detail {

inline void checked_add_counter(std::size_t& value, std::size_t delta,
                                const char* message) {
  if (delta > std::numeric_limits<std::size_t>::max() - value) {
    throw std::length_error(message);
  }
  value += delta;
}

struct SeedCandidateSet {
  std::vector<std::uint8_t> marked;
  std::size_t seed_count = 0U;
  std::size_t seed_occurrences = 0U;
  std::size_t unique_candidates = 0U;
};

inline SeedCandidateSet build_seed_candidates(const BwtByteIndex& index,
                                               std::string_view pattern,
                                               std::size_t max_edits) {
  if (pattern.empty() || max_edits >= pattern.size()) {
    throw std::logic_error(
        "seed candidate builder requires a non-trivial edit budget");
  }

  SeedCandidateSet result;
  const std::size_t text_size = index.text_size();
  result.marked.assign(index.row_count(), 0U);

  const std::size_t part_count = max_edits + 1U;
  result.seed_count = part_count;
  const std::size_t base_length = pattern.size() / part_count;
  const std::size_t longer_parts = pattern.size() % part_count;
  std::size_t seed_begin = 0U;

  for (std::size_t part = 0U; part < part_count; ++part) {
    const std::size_t seed_length =
        base_length + (part < longer_parts ? 1U : 0U);
    if (seed_length == 0U || seed_length > pattern.size() - seed_begin) {
      throw std::logic_error("seed candidate builder produced invalid partition");
    }

    const std::string_view seed = pattern.substr(seed_begin, seed_length);
    const std::vector<std::size_t> occurrences = index.locate(seed);
    checked_add_counter(result.seed_occurrences, occurrences.size(),
                        "seed candidate occurrence count overflow");

    for (const std::size_t occurrence : occurrences) {
      std::size_t lower = 0U;
      if (occurrence >= seed_begin) {
        const std::size_t center = occurrence - seed_begin;
        if (center > max_edits) {
          lower = center - max_edits;
        }
      }

      std::size_t upper = 0U;
      if (occurrence >= seed_begin) {
        const std::size_t center = occurrence - seed_begin;
        if (center > text_size) {
          continue;
        }
        upper = center + std::min(max_edits, text_size - center);
      } else {
        const std::size_t deficit = seed_begin - occurrence;
        if (deficit > max_edits) {
          continue;
        }
        upper = std::min(text_size, max_edits - deficit);
      }

      if (lower > text_size) {
        continue;
      }
      upper = std::min(upper, text_size);
      for (std::size_t start = lower; start <= upper; ++start) {
        result.marked[start] = 1U;
      }
    }
    seed_begin += seed_length;
  }

  if (seed_begin != pattern.size()) {
    throw std::logic_error("seed candidate partition did not cover pattern");
  }

  for (const std::uint8_t marked : result.marked) {
    if (marked != 0U) {
      checked_add_counter(result.unique_candidates, 1U,
                          "seed candidate count overflow");
    }
  }
  return result;
}

inline std::size_t verification_window_end(std::size_t text_size,
                                           std::size_t start,
                                           std::size_t pattern_size,
                                           std::size_t max_edits) {
  if (start > text_size) {
    throw std::logic_error("verification window start exceeds text size");
  }
  std::size_t max_length = text_size - start;
  if (max_edits <=
      std::numeric_limits<std::size_t>::max() - pattern_size) {
    max_length = std::min(max_length, pattern_size + max_edits);
  }
  return start + max_length;
}

inline bool verify_bounded_window(std::string_view pattern,
                                  std::string_view window,
                                  std::size_t max_edits,
                                  std::size_t& dp_cells) {
  if (pattern.empty() || max_edits >= pattern.size()) {
    throw std::logic_error(
        "bounded-window verifier requires a non-trivial edit budget");
  }
  const std::size_t min_length = pattern.size() - max_edits;
  if (window.size() < min_length) {
    return false;
  }

  std::vector<std::size_t> previous(window.size() + 1U);
  std::vector<std::size_t> current(window.size() + 1U);
  for (std::size_t column = 0U; column <= window.size(); ++column) {
    previous[column] = column;
  }

  for (std::size_t row = 1U; row <= pattern.size(); ++row) {
    current[0] = row;
    for (std::size_t column = 1U; column <= window.size(); ++column) {
      const std::size_t substitution =
          previous[column - 1U] +
          (pattern[row - 1U] == window[column - 1U] ? 0U : 1U);
      const std::size_t deletion = previous[column] + 1U;
      const std::size_t insertion = current[column - 1U] + 1U;
      current[column] = std::min({substitution, deletion, insertion});
      checked_add_counter(dp_cells, 1U,
                          "seed verification DP cell count overflow");
    }
    previous.swap(current);
  }

  for (std::size_t length = min_length; length <= window.size(); ++length) {
    if (previous[length] <= max_edits) {
      return true;
    }
  }
  return false;
}

}  // namespace algorithms::strings::detail
